import 'bootstrap/dist/css/bootstrap.min.css';
import React, { Component } from "react";
import { Card, Button, Row, Col } from "react-bootstrap";


export default class Design extends Component {
    constructor(props) {
        super(props);
        this.state = {};
    }

    handleShow = (value, type) => {
        console.log(value, type)
    }

    render() {
        return (
            <>
                <Row className="mt-5 mx-3">
                    <Col
                        className="mt-2"
                        sm={{ span: 12 }}
                        md={{ span: 6 }}
                        lg={{ span: 4 }}
                        xl={{ span: 3 }}
                    >
                        <Card>
                            <Card.Img variant="top" style={{ width:"75%" }} className="m-auto" src="https://gw.alipayobjects.com/zos/rmsportal/KDpgvguMpGfqaHPjicRK.svg" />
                            <Card.Body>
                                <Button variant="outline-primary" outline onClick={() => this.handleShow(1, "Ant")}>
                                    Ant Design
                                </Button>
                            </Card.Body>
                        </Card>
                    </Col>

                    <Col
                        className="mt-2"
                        sm={{ span: 12 }}
                        md={{ span: 6 }}
                        lg={{ span: 4 }}
                        xl={{ span: 3 }}
                    >
                        <Card>
                            <Card.Img variant="top" style={{ width:"75%" }} className="m-auto" src="data:image/svg+xml;base64," />
                            <Card.Body>
                                <Button
                                    variant="outline-primary"
                                    onClick={() => this.handleShow(2, "Bootstrap")}
                                >
                                    Bootstrap
                </Button>
                            </Card.Body>
                        </Card>
                    </Col>

                    <Col
                        className="mt-2"
                        sm={{ span: 12 }}
                        md={{ span: 6 }}
                        lg={{ span: 4 }}
                        xl={{ span: 3 }}
                    >
                        <Card>
                            <Card.Img variant="top" style={{ width:"75%" }} className="m-auto" src="https://react.semantic-ui.com/logo.png" />
                            <Card.Body>
                                <Button
                                    variant="outline-primary"
                                    onClick={() => this.handleShow(3, "Semantic")}
                                >
                                    Semantic UI
                </Button>
                            </Card.Body>
                        </Card>
                    </Col>


                    <Col
                        className="mt-2"
                        sm={{ span: 12 }}
                        md={{ span: 6 }}
                        lg={{ span: 4 }}
                        xl={{ span: 3 }}
                    >
                        <Card>
                            <Card.Img variant="top" style={{ width:"75%" }} className="m-auto" src="https://d259t2jj6zp7qm.cloudfront.net/images/v1479760233-trailhead_module_lightning_experience_basics_a0rcrt.png" />
                            <Card.Body>
                                <Button
                                    variant="outline-primary"
                                    onClick={() => this.handleShow(4, "Lightning")}
                                >
                                    Lightning Design
                </Button>
                            </Card.Body>
                        </Card>
                    </Col>



                </Row>
            </>
        );
    }
}